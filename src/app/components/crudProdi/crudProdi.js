...
export default function CRUDProdi() {
    ...
    const handleForm = async (e) => {
        ...
        try {
            let response
            if (formMode === "edit") {
                response = await axios.put(`/api/prodi/${formData.id_prodi}`, {jenjang, nama_prodi}, { 
                    headers: {
                        'Authorization' : `Bearer ${token}`
                    },
                    withCredentials: true 
                ...
                response = await axios.post(`/api/prodi`, {jenjang, nama_prodi}, { 
                    headers: {
                        'Authorization' : `Bearer ${token}`
                    },
                    withCredentials: true 
    ...
    const actionButton = (id_prodi, jenjang, nama_prodi) => {
        return (
            <div className='flex gap-3'>
                <Button onClick={() => openEditModal({ id_prodi, jenjang, nama_prodi })} className='bg-yellow-200' icon="pi pi-pen-to-square" text />
                <Button onClick={() => handleDelete(id_prodi)} className='bg-red-600' icon="pi pi-trash" text />
            </div>
        )
    }
    ...
    return (
        <div className='mx-10 mt-10'>
            <Modal size='4xl' show={openModal} onClose={() => setOpenModal(false)}>
                <ModalHeader>{formMode === 'create' ? "Tambahkan" : formMode === 'edit' ? "Edit" : ""} Prodi</ModalHeader>
                <ModalBody>
            ...
            <DataTable value={dataProdi} paginator showGridlines stripedRows rows={10} rowsPerPageOptions={[10, 15, 20, 25]} paginatorTemplate="PrevPageLink CurrentPageReport NextPageLink RowsPerPageDropdown" currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorRight resizableColumns size='large' emptyMessage="Prodi Tidak Tersedia." globalFilter={globalFilter} header={headerTable} tableStyle={{ minWidth: '50rem' }} className="rounded-md overflow-hidden shadow-md border border-gray-300 text-sm mb-32" paginatorClassName='bg-[#fafafa]'>
                ...
                <Column sortable field="jumlah_seleksi" header="Jumlah Seleksi"></Column>
                <Column header="Action" body={(rowData) => actionButton(rowData.id_prodi, rowData.jenjang, rowData.nama_prodi)}></Column>
            </DataTable>
        </div>
    )
}
